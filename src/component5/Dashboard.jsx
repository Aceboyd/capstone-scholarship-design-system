...
const Dashboard = () => {
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        {stats.map((stat, index) => (
          <div
            key={index}
            className={`p-3 sm:p-6 rounded-xl ${stat.color} ${stat.borderStyle || ''}`}
          >
            <div className="mb-1">
      ...
      <div className="mb-8">
        <div className="p-3 sm:p-6">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Program Insights</h2>
          <p className="text-gray-600 text-sm sm:text-base">Track key metrics across scholarship programs. Stay ahead of deadlines and application progress.</p>
          <div className="flex flex-wrap items-center space-x-2 sm:space-x-6 mt-4">
            <div className="flex items-center space-x-1 sm:space-x-2">
              <div className="w-3 h-3 bg-[#FFE7C7] rounded-full"></div>
              <span className="text-xs sm:text-sm text-[#E07800]">Closing soon</span>
            </div>
            <div className="flex items-center space-x-1 sm:space-x-2">
              <div className="w-3 h-3 bg-[#CDF3ED] rounded-full"></div>
              <span className="text-xs sm:text-sm text-[#12806E]">Open</span>
            </div>
            <div className="flex items-center space-x-1 sm:space-x-2">
              <div className="w-3 h-3 bg-[#F5DBDE] rounded-full"></div>
              <span className="text-xs sm:text-sm text-[#C30D24]">Deadline Alert</span>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs sm:text-sm">
            <thead className="bg-gray-200">
              <tr>
                <th className="px-4 sm:px-9 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Program Name</th>
                <th className="px-4 sm:px-9 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Applications</th>
                <th className="px-4 sm:px-9 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Avg. GPA</th>
                <th className="px-4 sm:px-9 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Recommended</th>
                <th className="px-4 sm:px-9 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Deadline</th>
                <th className="px-4 sm:px-9 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
                ...
                <tr key={index} className="hover:bg-gray-50 transition-colors">
                  <td className="px-3 sm:px-6 py-2 sm:py-4">
                    <div className="flex items-center">
                      <div className={`w-3 h-3 rounded-full mr-2 sm:mr-3 ${getStatusColor(program.status).split(' ')[0]}`}></div>
                      <span className="text-xs sm:text-sm font-medium text-gray-900">{program.name}</span>
                    </div>
                  </td>
                  <td className="px-3 sm:px-6 py-2 sm:py-4 text-xs sm:text-sm text-gray-900">{program.applications}</td>
                  <td className="px-3 sm:px-6 py-2 sm:py-4 text-xs sm:text-sm text-gray-900">{program.gpa}</td>
                  <td className="px-3 sm:px-6 py-2 sm:py-4 text-xs sm:text-sm text-gray-900">{program.recommended}</td>
                  <td className="px-3 sm:px-6 py-2 sm:py-4 text-xs sm:text-sm text-gray-900">{program.deadline}</td>
                  <td className="px-3 sm:px-6 py-2 sm:py-4">
        ...
        <div className="p-3 sm:p-6 text-center mt-[-5px]">
          <button className="inline-flex items-center px-4 sm:px-10 py-2 sm:py-4 border text-xs sm:text-sm font-medium text-gray-700 rounded-lg hover:bg-gray-300 transition-colors">
            View more Programs
      ...
      <div className="rounded-xl">
        <div className="p-3 sm:p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <div>
              <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Alerts and Reminders</h2>
              <p className="text-gray-600 text-sm sm:text-base">Stay informed about upcoming deadlines and incomplete reviews.</p>
            </div>
        ...
        <div className="p-3 sm:p-6 space-y-3 sm:space-y-4">
          {alerts.map((alert, index) => (
            <div key={index} className={`flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-6 p-3 sm:p-6 ${alert.color} rounded-lg`}>
              <div className="flex-1">
                <div className="flex items-center space-x-2 sm:space-x-4 mb-1">
                  <span className={`text-xs sm:text-sm font-medium ${alert.textColor}`}>{alert.type}</span>
                </div>
                <p className={`text-xs sm:text-sm ${alert.textColor}`}>{alert.message}</p>
              </div>
              <button className={`px-3 sm:px-4 py-1 sm:py-2 ${alert.buttonColor} rounded-lg hover:bg-opacity-80 transition-colors text-xs sm:text-sm font-medium flex items-center`}> 
      ...
      <div className="p-3 sm:p-6 text-center mt-[-30px]">
        <button className="inline-flex items-center px-4 sm:px-10 py-2 sm:py-4 border text-xs sm:text-sm font-medium text-gray-700 rounded-lg hover:bg-gray-300 transition-colors">
          View all Alerts
          <ArrowRight size={16} className="ml-2" />
        </button>
