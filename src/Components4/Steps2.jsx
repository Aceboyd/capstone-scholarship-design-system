...
const Steps2 = ({ progress, step2Progress, appStatus }) => {
  const stepDetails = [
    ...
    },
    {
      ...
      borderColor: step2Progress > 0 ? "border-[#BFDBFE]" : "border-[#6B7280]",
      numberColor: step2Progress > 0 ? "text-[#BFDBFE]" : "text-[#6B7280]",
    },
    {
      label: "STEP 3",
          ...
          {stepDetails.map((step, index) => (
            <React.Fragment key={index}>
              ...
              <div className="flex flex-col w-1/5 h-40 mt-7">
                <div className="flex w-[90%] h-1 rounded-full bg-gray-200">
                  <div
                    className={`h-full rounded-full ${step.style2}`}
                    style={step.label === "STEP 2" ? { width: `${step2Progress}%` } : { width: step.label === "STEP 1" ? "100%" : "0%" }}
                  ></div>
                </div>
