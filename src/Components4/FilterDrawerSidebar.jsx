...
const FilterSection = ({ title, children, defaultOpen = false }) => {
  ...
  return (
    <div className="space-y-2">
      <button
        className="flex w-full cursor-pointer items-center justify-between font-bold text-gray-800 focus:outline-none sm:text-lg"
...
export default function FilterDrawerSidebar({
  isVisible,
  selectedFilters,
  setSelectedFilters,
}) {
  ...
  return (
    <aside
      role="complementary"
      aria-label="Filters"
      className={`mx fixed top-18 left-0 z-30 h-[88%] w-full overflow-y-auto p-4`}
    >
      {/* Selected Filters */}
      <div className="flex items-center justify-between pb-2 sm:pb-1">
        <div className="flex items-center gap-2">
          <CiFilter className="h-5 w-5 text-gray-300 sm:h-4 sm:w-4" />
          <p className="text-base text-gray-300 sm:text-sm">Selected Filters</p>
        </div>
        <button
          onClick={resetFilters}
          className="cursor-pointer rounded-md text-base font-bold text-gray-300 transition-colors hover:text-gray-100 focus:outline-none sm:text-sm"
