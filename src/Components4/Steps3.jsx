...
const stepDetails = [
  ...
  { label: "STEP 3", borderstyle: "border-blue-700", title: "Supporting Documents", imagestep: number3Image, style: "mt-1 sm:text-[8px] md:text-[10px] lg:text-xs text-[#0000FE] border border-[#0000FE] rounded-full px-1 lg:px-2 w-[80%] lg:w-[60%] text-center", status: "In Progress" },
...
const Steps3 = ({ progress, appStatus }) => {
  const stepDetails = [
    ...
    },
    {
      ...
      borderColor: progress > 40 ? "border-[#BFDBFE]" : "border-[#6B7280]",
      numberColor: progress > 40 ? "border-[#BFDBFE]" : "border-[#6B7280]",
    },
    {
      label: "STEP 4",
                    ...
                    style={{
                      width:
                        step.label === "STEP 1" || step.label === "STEP 2"
                          ? "100%"
                          : step.label === "STEP 3"
                          ? `${(progress - 40) * 5}%` // Scale 0-20% for Step 3
                          : "0%",
                    }}
                  ></div>
                </div>
                <span className="text-sm text-gray-500 mt-1">{step.label}</span>
                <span className="text-[8px] lg:text-[10px] xl:text-sm text-start text-gray-700 whitespace-pre">
                  {step.title}
                </span>
                <span className={`${step.style} text-[8px]`}>{step.status}</span>
              </div>
